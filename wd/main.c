...
main(int argc, char* argv[])
{
  ...
  for (size_t ix = 0; ix < total_lines - 1; ix++) {
    ...
    size_t line_offset, curr_max_load_lines;
...
#pragma omp parallel default(shared) firstprivate(line_offset) reduction(+ : counting[:3465])
      {
#pragma omp for
        for (size_t ixc = 0; ixc < curr_max_load_lines; ixc++) {
